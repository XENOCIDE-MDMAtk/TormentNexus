...
REM ==============================================
REM Borg AIOS - Go Sidecar Startup
...
REM -- Configuration ----------------------------------
set GO_PORT=%BORG_GO_PORT%
if "%GO_PORT%"=="" set GO_PORT=4300
set GO_HOST=%BORG_GO_HOST%
if "%GO_HOST%"=="" set GO_HOST=127.0.0.1
set GO_CONFIG=%BORG_GO_CONFIG%
if "%GO_CONFIG%"=="" set GO_CONFIG=%USERPROFILE%\.borg-go
...
cd go
go build -ldflags "-s -w -X github.com/borghq/borg-go/internal/buildinfo.Version=%VER%" -buildvcs=false -o ..\bin\borg.exe ./cmd/borg 2>nul
if errorlevel 1 (
...
curl -s -o nul -w "%%{http_code}" http://%GO_HOST%:%GO_PORT%/health > "%TEMP%\borg_go_check.txt" 2>nul

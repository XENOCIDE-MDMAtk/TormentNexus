# BORG FEATURE ASSESSMENT & NEXT PRIORITIES
*Derived from: (1) Borg codebase audit (2) 13,503 bookmark ecosystem intelligence*
*Date: 2026-05-08 | Version: 1.0.0-alpha.53*

---

## WHAT BORG IS (Actual, Not Aspirational)

Borg is a **local-first cognitive control plane** — a Go+TypeScript modular monolith that coordinates multi-agent LLM workflows, MCP tool routing, provider failover, memory persistence, and operator observability. It currently runs as two cooperating processes:
...
- **SQLite storage**: metamcp.db (35MB), .borg/agent_memory/ (14,726 memories), .borg/memory/ (13,478 contexts)
...
| **Knowledge Graph** | ✅ | ✅ | Stub | `@borg/memory` exports `GraphNode`/`GraphEdge` **interfaces only** — actual implementations are `undefined`. `RepoGraphService` builds file-level import graphs but NOT semantic entity graphs |
...
| **Skill Registry** | ✅ | ✅ | Beta | SKILL.md discovery + CRUD + search. **No /evolve command**, no win-rate tracking, no auto-retirement. 0 skills in `.borg/skills/` |
...
| Stack Layer | Systems | Saturation | Borg Coverage |
|-------------|---------|-----------|---------------|
...
- Self-Mod + Tools: 2.7% deficit → Borg has tools but self-mod is disconnected
- Memory + Tools: 1.7% deficit → Borg's memory doesn't inform tool selection
...
Ranked by: (1) evidence from ecosystem data, (2) Borg's existing foundation to build on, (3) leverage — how much it unlocks downstream
...
**Borg foundation:** L1 Scratchpad + L2 Vault already work. 14,726 memories already stored.
...
**Borg foundation:** HealerService (TS) + healer.go (Go) already do error→diagnosis→fix suggestion
...
**Evidence:** 37x enrichment (skill evolution), listed in Borg's own TODO
**Borg foundation:** MCP Decision System already has the exact architecture (ranked search, auto-load, LRU eviction, profiles). SkillRegistry already discovers SKILL.md files.
...
**Borg foundation:** ContextHarvester + Groomer already compact context. Memory hydration already injects L2 into L1 on session start.
...
**Borg foundation:** PairOrchestrator has the state machine. Council has debate/consensus. HumanVetoService exists.
...
**Borg foundation:** Both memory and tool selection are mature systems. They just don't talk.
**What to build:**
...
**Borg foundation:** RepoGraphService builds file-level graphs. KnowledgeService has graph interfaces. `@borg/memory` exports GraphNode/GraphEdge types.
...
**Borg foundation:** SkillRegistry + SkillAssimilationService + DarwinService all exist in pieces.
...
**Borg foundation:** HumanVetoService exists. A2A broker works. KnowledgeService has graph types.
...
The data says these layers are OVERBUILT in the ecosystem. Borg should **consume** them, not compete:

| Don't Build | Reason | Instead |
|-------------|--------|---------|
| Another agent framework | 52% saturated | Borg's swarm/council is sufficient — focus on making it reliable |
| Another MCP server | 66% saturated | Borg should ROUTE existing servers, not create new ones |
| Another dashboard framework | 52% saturated | 91 pages is enough — focus on data quality, not more pages |
| Another CLI harness | Crowded | Borg should COORDINATE existing harnesses via parity aliases |

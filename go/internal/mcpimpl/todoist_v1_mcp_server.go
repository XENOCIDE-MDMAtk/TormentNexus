...
func HandleListProjects_todoist_v1_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleCreateTask_todoist_v1_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	token := os.Getenv("TODOIST_API_TOKEN")
	if token == "" {
		return err("TODOIST_API_TOKEN environment variable not set")
}

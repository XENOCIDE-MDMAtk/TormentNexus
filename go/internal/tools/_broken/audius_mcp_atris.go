...
func HandleSearchTracks_audius_mcp_atris(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func joinStrings_audius_mcp_atris(items []string, sep string) string {
	result := ""
	for i, s := range items {
		if i > 0 {
			result += sep

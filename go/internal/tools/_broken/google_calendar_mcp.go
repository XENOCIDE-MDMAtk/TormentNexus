...
func HandleListEvents_google_calendar_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleCreateEvent_google_calendar_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	token := os.Getenv("GOOGLE_CALENDAR_TOKEN")
	calendarID, _ :=getString(args, "calendarId")
	if calendarID == "" {
		calendarID = "primary"

...
func HandleListPods_mcp_k8s_go(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleListNodes_mcp_k8s_go(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	token, e := os.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/token")
	if e != nil {
		return err("failed to read service account token: " + e.Error())
}

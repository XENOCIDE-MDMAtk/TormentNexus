...
func HandleSearch_mcp_google_search(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func joinStrings_mcp_google_search(strs []string, sep string) string {
	result := ""
	for i, s := range strs {
		if i > 0 {
			result += sep

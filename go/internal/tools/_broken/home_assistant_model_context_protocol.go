...
func HandleListStates_home_assistant_model_context_protocol(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleCallService_home_assistant_model_context_protocol(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	url := os.Getenv("HOME_ASSISTANT_URL")
	token := os.Getenv("HOME_ASSISTANT_TOKEN")
	if url == "" || token == "" {
		return err("HOME_ASSISTANT_URL and HOME_ASSISTANT_TOKEN must be set")

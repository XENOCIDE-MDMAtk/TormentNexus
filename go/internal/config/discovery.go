...
// ServiceDiscovery holds resolved endpoints for all Borg services.
...
// DefaultServiceDiscovery returns the standard Borg service topology.
func DefaultServiceDiscovery() ServiceDiscovery {
	...
	if v := os.Getenv("BORG_GO_PORT"); v != "" {
		if p, err := strconv.Atoi(v); err == nil && p > 0 {
			sd.GoSidecarPort = p
		}
	}

	if v := strings.TrimSpace(os.Getenv("BORG_TRPC_UPSTREAM")); v != "" {
		sd.TRPCUpstreamURLs = append([]string{v}, sd.TRPCUpstreamURLs...)
	}

	if v := os.Getenv("BORG_BRIDGE_PORT"); v != "" {
		if p, err := strconv.Atoi(v); err == nil && p > 0 {
			sd.BridgePort = p
		}
	}

	if v := os.Getenv("BORG_DASHBOARD_PORT"); v != "" {
		if p, err := strconv.Atoi(v); err == nil && p > 0 {
			sd.DashboardPort = p
		}
	}

	if v := os.Getenv("BORG_DASHBOARD_HOST"); v != "" {
		sd.DashboardHost = v
	}
